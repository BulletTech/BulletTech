...
至于扩展性，搭配M1芯片的Mac中，Mac mini的接口数量是最多、最全的之一（如下图所示）。该机型最高支持同时连接一台6K (60Hz) 的显示器和一台4K (60Hz) 的显示器。本人使用的是LG 27UL850的显示器，实测下来使用雷雳 3和HDMI 2.0 端口都能达到4K (60Hz) 的输出，除了显示器偶尔黑屏外，日常办公体验十分优秀。如需外接更多的设备，可按需另外购入扩展坞。
