...
通过肉眼，我们可以发现异常值在第6000个观测值附近。并且那一块并没有出现极值。因此，可以说对于这种周期性较强的时间序列，极值可能也是周期的一部分。我们需要识别的是某个与其他周期不同的小区间。

## 5 Matrixprofile算法

那么，如何识别出周期层面的异常区间呢？我们可能会想，我们先确定一个区间，然后看看这个区间和其他同样长度的区间是否不同。  
我们可能需要一个循环，滑动窗口的进行判断。题目好心的提示我们有这样一个包：matrixprofile。
