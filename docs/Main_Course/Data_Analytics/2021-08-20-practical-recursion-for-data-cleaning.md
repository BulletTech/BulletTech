...
在日常数据分析的工作中，我们收集到的原始数据有时并不是整齐的表格形式，例如在爬取网页或者爬取api里的数据时，结果往往是以XML或者JSOM（类似Python中的字典）格式返回，并且层层嵌套。就像如下这样的JSON格式：
...
没错，递归的本质就是"复读"，在计算机编程中，递归就是通过函数调用自身，把问题转化成解决一个过程相似，但是规模较小的问题，直到到达边界条件即小化的问题。一个完整的递归函数一般具有以下三要素：
